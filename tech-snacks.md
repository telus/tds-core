...
* This repository's git history contains somewhat large binary files including Sketch documents and PNGs. Let's set up Git LFS for "_.png" and "_.sketch" patterns in order to retroactively apply large file storage to those files. The benefit of Git LFS is to only download those files at checkout, and not fetch or clone.
