...
component, which "listens for matches to a CSS media query and renders its child component based on whether the query matches or not." Generally, using Sass-based breakpoints is recommended. However, you may want to use this implementation if you want to do something programmatically. For example, changing a prop or swapping a component on resize.
