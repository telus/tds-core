...
<link
  rel="stylesheet"
  type="text/css"
  href="https://cdn.telus.digital/thorium/core/v0.3.0/thorium.min.css">
```
...
Thorium uses <a href="http://semver.org/" target="_blank">semantic versioning</a> to number its releases. Basically, a number like `0.3.0` has three parts from left to right:
...
The version constraints in your package.json file control which version of Thorium gets pulled in. NPM by default will choose a constraint like `^0.3.0` when the `--save` command line flag is used. This allows major version changes (bug fixes and new backwards-compatible features).
